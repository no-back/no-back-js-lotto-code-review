...
describe("구매한 로또 UI Test", () => {
  ...
  it("로또를 구입한 후 입력된 로또 구입 금액으로 발급한 로또에 관란 섹션이 화면에 표시한다.", () => {
    ...
    cy.get("[data-purchased-lotto='section']")
  ...
  it("발급한 로또의 개수가 일치한다.", () => {
    cy.get("[data-purchase-form='input']").type(purchaseAmount).type("{enter}");
    cy.get('[data-purchased-lotto="amount"]').should(
      "have.text",
  ...
  it("번호보기 토글이 비활성화되어 있는 상태에서 토글을 누르면, 로또 아이콘이 세로로 배치되고 로또 번호가 표시된다.", () => {
    cy.get("[data-purchase-form='input']").type(purchaseAmount).type("{enter}");
    cy.get('[data-toggle-switch="switch"]').click();
    cy.get('[data-lotto-number-toggle="button"]').should("be.checked");
    cy.get('[data-lotto-ticket="container"]').should("have.class", "flex-col");
    cy.get('[data-lotto-numbers="span"]').should("be.visible");
  });
  it("표시된 로또 번호의 개수, 중복여부, 범위를 검사한다", () => {
    cy.get("[data-purchase-form='input']").type(purchaseAmount).type("{enter}");
    cy.get('[data-toggle-switch="switch"]').click();
    cy.get('[data-lotto-numbers="span"]').each(($lotto) => {
  ...
  it("로또를 재구입할 경우 기존의 구매한 로또를 삭제하고 새로 구매한 로또를 보여준다.", () => {
    cy.get("[data-purchase-form='input']").type(purchaseAmount).type("{enter}");
    cy.get("[data-purchase-form='input']").clear();
    cy.get("[data-purchase-form='input']")
      .type(newPurchaseAmount)
      .type("{enter}");
    cy.get('[data-purchased-lotto="amount"]').should(
      "have.text",
